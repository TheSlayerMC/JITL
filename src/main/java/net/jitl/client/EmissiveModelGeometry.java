...
public class EmissiveModelGeometry implements IUnbakedGeometry<EmissiveModelGeometry> {
    ...
    private final BiFunction<EmissiveModelGeometry, IGeometryBakingContext, Material> particleMaterialSupplier;

    public EmissiveModelGeometry(@Nullable BlockModel emissiveModel, @Nullable BlockModel normalModel, BiFunction<EmissiveModelGeometry, IGeometryBakingContext, Material> particleMaterialSupplier) {
    ...
    public BakedModel bake(IGeometryBakingContext owner, ModelBakery bakery, Function<Material, TextureAtlasSprite> spriteGetter, ModelState modelTransform, ItemOverrides overrides, ResourceLocation modelLocation) {
        ...
        if (emissiveModel != null) {
            ...
            if (bakedEmissive instanceof SimpleBakedModel) {
                List<BakedQuad> quads = bakedEmissive.getQuads(null, null, RandHelper.RAND, ModelData.EMPTY);
                quads = lightQuads(quads);

                Map<Direction, List<BakedQuad>> faceQuads = new EnumMap<>(Direction.class);

                for (Direction dir : Direction.values()) {
                    faceQuads.put(dir, lightQuads(bakedEmissive.getQuads(null, dir, RandHelper.RAND, ModelData.EMPTY)));
                }
    ...
    public static class Loader implements IGeometryLoader<EmissiveModelGeometry> {
        public static final Loader INSTANCE = new Loader();

        private Loader() {}

        @Override
        public EmissiveModelGeometry read(JsonObject modelContents, JsonDeserializationContext deserializationContext) {
            ...
            BiFunction<EmissiveModelGeometry, IGeometryBakingContext, Material> particlesSupplier;
    ...
    private static BiFunction<EmissiveModelGeometry, IGeometryBakingContext, Material> createParticleRenderMaterialSupplier(ModelType type) {
