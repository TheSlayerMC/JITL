        ...
        if(player != null && !player.isCreative() && !player.isSpectator()) {
            player.getCapability(PlayerEssenceProvider.PLAYER_ESSENCE).ifPresent(essence -> {
                ...
                if (!minecraft.options.hideGui && transparency > 0 && !player.isSpectator()) {
                    ...
                    GuiComponent.blit(poseStack, x, y, 0, backgroundVOffset, 81, barHeight, 81, texHeight);
                    ...
                    if (cooldownActive) {
                        ...
                        GuiComponent.blit(poseStack, x, y, 0, 0, 81, barHeight, 81, texHeight);
                    } else {
                        int i = (int) ((currentEssence / maxEssence) * 81);
                        GuiComponent.blit(poseStack, x, y, 0, 0, i, barHeight, 81, texHeight);
                    }

                    if (burnoutTransparency > 0) {
                        RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, burnoutTransparency - addedAlpha);
                        GuiComponent.blit(poseStack, x, y, 0, burnoutVOffset, 81, barHeight, 81, texHeight);
                    }
                    poseStack.popPose();
                }
            });
