...
import net.jitl.client.gui.EntryButton;
...
public class LoreScrollScreen extends Screen {
    ...
    private int mouseX;
    private int mouseY;
    private final float initialMouseClickY = -2.0F;
    private final float initialMouseClickX = -2.0F;
    ...
    private final float blInitialMouseClickY = -2.0F;
    private float blscrollFactor;
    private float blscrollDistance;
    private int catButtonsHeight;

    public LoreScrollScreen(ScrollCategory scrollCategory) {
        super(Component.translatable("jitl.screen.lore_scroll"));
        this.currentCategory = scrollCategory;
        categorySize = currentCategory.getCategorySize() * 256;
    }

    public LoreScrollScreen() {
        super(Component.translatable("jitl.screen.lore_scroll"));
    ...
    public void init() {
        ...
        for (int i = 0; i < ScrollAPI.getCategoryMap().size() * 1000; i += 1000) {
            ...
            //FIXME add category button
            //addRenderableWidget(new GuiCategoryButton(i, x0 + 25, y0 + 20 + i * buttonHeight + i * border, buttonWidth, buttonHeight, category));
    ...
    @Override
    public void render(GuiGraphics poseStack_, int mouseX, int mouseY, float partialTick_) {
        renderBackground(poseStack_, mouseX, mouseY, partialTick_);
        ...
        super.render(poseStack_, mouseX, mouseY, partialTick_);
...
//    private void renderEntryInfo(GuiGraphics poseStack, Font font, EntryButton button) {
//
//        List<String> textLines = new ArrayList<>();
//        textLines.add(Component.translatable(button.entry.getTitleKey()).getString());
//        assert button.entry.getCommentKey() != null;
//        textLines.add(Component.translatable(button.entry.getCommentKey()).getString());
//
