...
public class LuniumShieldRenderer implements SpecialModelRenderer<DataComponentMap> {

    private final ShieldModel model;

    public LuniumShieldRenderer(ShieldModel model) {
        this.model = model;
    }
    ...
    public void render(@Nullable DataComponentMap map, ItemDisplayContext ctx, PoseStack pose, MultiBufferSource buff, int alp, int col, boolean bol) {
        ...
        VertexConsumer vertexconsumer = buff.getBuffer(RenderType.entityCutoutNoCull(JITL.rl("textures/shield/" + "lunium" + "_shield.png")));
    ...
    public record Unbaked() implements SpecialModelRenderer.Unbaked {
        public static final LuniumShieldRenderer.Unbaked INSTANCE = new LuniumShieldRenderer.Unbaked();
        public static final MapCodec<Unbaked> MAP_CODEC;

        public MapCodec<LuniumShieldRenderer.Unbaked> type() {
            return MAP_CODEC;
        }

        public SpecialModelRenderer<?> bake(EntityModelSet p_387269_) {
            return new LuniumShieldRenderer(new ShieldModel(p_387269_.bakeLayer(ModelLayers.SHIELD)));
        }
