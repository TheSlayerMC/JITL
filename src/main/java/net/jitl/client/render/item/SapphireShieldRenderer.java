...
public class SapphireShieldRenderer implements SpecialModelRenderer<DataComponentMap> {
    ...
    public SapphireShieldRenderer(SpriteGetter materials, ShieldModel model) {
        this.model = model;
    ...
    public record Unbaked() implements SpecialModelRenderer.Unbaked<DataComponentMap> {
        ...
        public SpecialModelRenderer bake(SpecialModelRenderer.BakingContext c) {
            return new SapphireShieldRenderer(c.sprites(), new ShieldModel(c.entityModelSet().bakeLayer(ModelLayers.SHIELD)));
        }
