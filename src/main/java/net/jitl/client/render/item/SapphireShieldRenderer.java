...
public class SapphireShieldRenderer implements SpecialModelRenderer<DataComponentMap> {
    ...
    public SapphireShieldRenderer(ShieldModel model) {
        this.model = model;
    }
    ...
    public void render(@Nullable DataComponentMap map, ItemDisplayContext ctx, PoseStack pose, MultiBufferSource buff, int alp, int col, boolean bol) {
        ...
        VertexConsumer vertexconsumer = buff.getBuffer(RenderType.entityCutoutNoCull(JITL.rl("textures/shield/" + "sapphire" + "_shield.png")));
    ...
    public record Unbaked() implements SpecialModelRenderer.Unbaked {
        public static final SapphireShieldRenderer.Unbaked INSTANCE = new SapphireShieldRenderer.Unbaked();
        public static final MapCodec<Unbaked> MAP_CODEC;

        public MapCodec<SapphireShieldRenderer.Unbaked> type() {
            return MAP_CODEC;
        }

        public SpecialModelRenderer<?> bake(EntityModelSet p_387269_) {
            return new SapphireShieldRenderer(new ShieldModel(p_387269_.bakeLayer(ModelLayers.SHIELD)));
        }
