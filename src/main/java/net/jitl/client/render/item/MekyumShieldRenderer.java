...
public class MekyumShieldRenderer implements SpecialModelRenderer<DataComponentMap> {
    ...
    public MekyumShieldRenderer(SpriteGetter materials, ShieldModel model) {
        this.model = model;
    ...
    public void submit(@Nullable DataComponentMap typedDataComponents, PoseStack pose, SubmitNodeCollector submitNodeCollector, int i, int i1, boolean b, int i2) {
        ...
        SpriteId material = new SpriteId(Sheets.SHIELD_SHEET, JITL.rl("entity/shield/mekyum_shield"));
    ...
    public record Unbaked() implements SpecialModelRenderer.Unbaked<DataComponentMap> {
        ...
        public SpecialModelRenderer bake(SpecialModelRenderer.BakingContext c) {
            return new MekyumShieldRenderer(c.sprites(), new ShieldModel(c.entityModelSet().bakeLayer(ModelLayers.SHIELD)));
        }
