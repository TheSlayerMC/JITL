...
public class GorbiteShieldRenderer implements SpecialModelRenderer<DataComponentMap> {

    private final ShieldModel model;

    public GorbiteShieldRenderer(ShieldModel model) {
        this.model = model;
    }
    ...
    public void render(@Nullable DataComponentMap map, ItemDisplayContext ctx, PoseStack pose, MultiBufferSource buff, int alp, int col, boolean bol) {
        ...
        VertexConsumer vertexconsumer = buff.getBuffer(RenderType.entityCutoutNoCull(JITL.rl("textures/shield/" + "gorbite" + "_shield.png")));
    ...
    public record Unbaked() implements SpecialModelRenderer.Unbaked {
        public static final GorbiteShieldRenderer.Unbaked INSTANCE = new GorbiteShieldRenderer.Unbaked();
        public static final MapCodec<Unbaked> MAP_CODEC;

        public MapCodec<GorbiteShieldRenderer.Unbaked> type() {
            return MAP_CODEC;
        }

        public SpecialModelRenderer<?> bake(EntityModelSet p_387269_) {
            return new GorbiteShieldRenderer(new ShieldModel(p_387269_.bakeLayer(ModelLayers.SHIELD)));
        }
