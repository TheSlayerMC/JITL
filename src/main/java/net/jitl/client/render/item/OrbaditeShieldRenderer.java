...
public class OrbaditeShieldRenderer implements SpecialModelRenderer<DataComponentMap> {
    ...
    public OrbaditeShieldRenderer(SpriteGetter materials, ShieldModel model) {
        this.model = model;
    ...
    public void submit(@Nullable DataComponentMap typedDataComponents, PoseStack pose, SubmitNodeCollector submitNodeCollector, int i, int i1, boolean b, int i2) {
        ...
        SpriteId material = new SpriteId(Sheets.SHIELD_SHEET, JITL.rl("entity/shield/orbadite_shield"));
    ...
    public record Unbaked() implements SpecialModelRenderer.Unbaked<DataComponentMap> {
        ...
        public SpecialModelRenderer bake(SpecialModelRenderer.BakingContext c) {
            return new OrbaditeShieldRenderer(c.sprites(), new ShieldModel(c.entityModelSet().bakeLayer(ModelLayers.SHIELD)));
        }
