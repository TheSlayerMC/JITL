...
public class StoronShieldRenderer implements SpecialModelRenderer<DataComponentMap> {
    ...
    public StoronShieldRenderer(ShieldModel model) {
        this.model = model;
    }
    ...
    public void render(@Nullable DataComponentMap map, ItemDisplayContext ctx, PoseStack pose, MultiBufferSource buff, int alp, int col, boolean bol) {
        ...
        VertexConsumer vertexconsumer = buff.getBuffer(RenderType.entityCutoutNoCull(JITL.rl("textures/shield/" + "storon" + "_shield.png")));
    ...
    public record Unbaked() implements SpecialModelRenderer.Unbaked {
        public static final StoronShieldRenderer.Unbaked INSTANCE = new StoronShieldRenderer.Unbaked();
        public static final MapCodec<Unbaked> MAP_CODEC;

        public MapCodec<StoronShieldRenderer.Unbaked> type() {
            return MAP_CODEC;
        }

        public SpecialModelRenderer<?> bake(EntityModelSet p_387269_) {
            return new StoronShieldRenderer(new ShieldModel(p_387269_.bakeLayer(ModelLayers.SHIELD)));
        }
