...
public class DesShieldRenderer implements SpecialModelRenderer<DataComponentMap> {
    ...
    public DesShieldRenderer(ShieldModel model) {
        this.model = model;
    }
    ...
    public void render(@Nullable DataComponentMap map, ItemDisplayContext ctx, PoseStack pose, MultiBufferSource buff, int alp, int col, boolean bol) {
        ...
        VertexConsumer vertexconsumer = buff.getBuffer(RenderType.entityCutoutNoCull(JITL.rl("textures/shield/" + "des" + "_shield.png")));
    ...
    public record Unbaked() implements SpecialModelRenderer.Unbaked {
        public static final DesShieldRenderer.Unbaked INSTANCE = new DesShieldRenderer.Unbaked();
        public static final MapCodec<Unbaked> MAP_CODEC;

        public MapCodec<DesShieldRenderer.Unbaked> type() {
            return MAP_CODEC;
        }

        public SpecialModelRenderer<?> bake(EntityModelSet p_387269_) {
            return new DesShieldRenderer(new ShieldModel(p_387269_.bakeLayer(ModelLayers.SHIELD)));
        }
