...
public class ShadiumShieldRenderer implements SpecialModelRenderer<DataComponentMap> {
    ...
    public ShadiumShieldRenderer(SpriteGetter materials, ShieldModel model) {
        this.model = model;
    ...
    public void submit(@Nullable DataComponentMap typedDataComponents, PoseStack pose, SubmitNodeCollector submitNodeCollector, int i, int i1, boolean b, int i2) {
        ...
        SpriteId material = new SpriteId(Sheets.SHIELD_SHEET, JITL.rl("entity/shield/shadium_shield"));
    ...
    public record Unbaked() implements SpecialModelRenderer.Unbaked<DataComponentMap> {
        ...
        public SpecialModelRenderer bake(SpecialModelRenderer.BakingContext c) {
            return new ShadiumShieldRenderer(c.sprites(), new ShieldModel(c.entityModelSet().bakeLayer(ModelLayers.SHIELD)));
        }
