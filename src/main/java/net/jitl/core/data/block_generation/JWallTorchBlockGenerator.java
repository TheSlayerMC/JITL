...
public class JWallTorchBlockGenerator extends BasicFileGenerator {

    protected BufferedWriter blockModelWriter, blockstateWriter, itemModelWriter;

    public void generate() {
        for(String name : JBlocks.wallTorchBlockName) {
    ...
    public void getBlockModel(String modID, String name) {
        ...
        writeToBlockModelFile("  \"parent\": \"minecraft:block/template_torch_wall\",");
        writeToBlockModelFile("  \"render_type\": \"cutout\",");
        writeToBlockModelFile("  \"textures\": {");
        writeToBlockModelFile("    \"torch\": \"" + modID + ":" + "block/" + JBlockProperties.getTextureFromName(name) + "\"");
        writeToBlockModelFile("  }");
        writeToBlockModelFile("}");
    }
