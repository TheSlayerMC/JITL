...
public class JEntities {
    ...
    public static final DeferredHolder<EntityType<?>, EntityType<Gunsmith>> GUNSMITH_TYPE = registerEntity(Gunsmith::new, "gunsmith", "Gunsmith", 1F, 2F, OVERWORLD_COLOR, TRADER_COLOR, MobCategory.CREATURE);
    ...
    private static <T extends Mob> DeferredHolder<EntityType<?>, EntityType<T>> registerEntity(EntityType.EntityFactory<T> factory, String name, String lang, float width, float height) {
        DeferredHolder<EntityType<?>, EntityType<T>> entity = REGISTRY.register(name, () -> EntityType.Builder.of(factory, MobCategory.AMBIENT).sized(width, height).build(ResourceLocation.fromNamespaceAndPath(JITL.MODID, name).toString()));
        entityName.add(name);
        entityLangName.add(lang);
        return entity;
    }
