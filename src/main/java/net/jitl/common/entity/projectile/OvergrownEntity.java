...
public class OvergrownEntity extends ThrowableProjectile {

    public OvergrownEntity(EntityType<OvergrownEntity> type, Level world) {
        super(type, world);
    }

    public OvergrownEntity(Level world, LivingEntity thrower) {
        super(JEntities.OVERGROWN_TYPE.get(), thrower, world);
    }
    ...
    protected void onHitEntity(@NotNull EntityHitResult res) {
        ...
        if(entity instanceof LivingEntity && entity.hurt(this.damageSources().thrown(this, this.getOwner()), getDamage())) {
            MobEffectInstance effectInstance = new MobEffectInstance(MobEffects.BLINDNESS, 100);
