...
public class Auron extends JVillagerEntity {

    public Auron(EntityType<? extends JVillagerEntity> type, Level worldIn) {
        super(type, worldIn);
    }

    @Override
    public Holder<VillagerProfession> getVillagerProfession() {
        return JVillagerProfession.AURON;
    }
    ...
    public @NotNull InteractionResult mobInteract(@NotNull Player player, @NotNull InteractionHand playerHand) {
        switch (random.nextInt(3)) {
            case 0 -> ChatUtils.addDialogStyleChat(player, "jitl.trader.auron1");
            case 1 -> ChatUtils.addDialogStyleChat(player, "jitl.trader.auron2");
            case 2 -> ChatUtils.addDialogStyleChat(player, "jitl.trader.auron3");
        }
    ...
    private final RawAnimation IDLE = RawAnimation.begin().thenLoop("animation.auron.idle");
