...
public class SenterianTeleporter extends BaseTeleporter {
    ...
    public PortalInfo getPortalInfo(Entity entity, ServerLevel destWorld, Function<ServerLevel, PortalInfo> defaultPortalInfo) {
        ...
        if(destWorld.dimension() == Dimensions.SENTERIAN) {
    ...
    private PortalInfo repairPortal(ServerLevel level, BlockPos position, Entity entity) {
        ...
        BlockPos pos = null;
        Block tile = JBlocks.SENTERIAN_FLOOR.get();
        loop: for(int x = position.getX() - 11; x < position.getX() + 11; x++) for(int y = surface - 7; y < surface + 7; y++) for(int z = position.getZ() - 11; z < position.getZ() + 11; z++) if(level.getBlockState(pos = new BlockPos(x, y, z)).is(tile)) {
            Block brick = JBlocks.SENTERIAN_FLOOR.get();
        ...
        for (int x = -2; x < 3; x++)
            for (int z = -2; z < 3; z++) {
                if (Math.abs(x) < 2 && Math.abs(z) < 2)
                    ...
                    if (x == -2)
                        level.setBlock(pos.offset(x, 0, z), portal_frame.defaultBlockState().setValue(HorizontalDirectionalBlock.FACING, Direction.EAST).setValue(SenterianPortalFrameBlock.HAS_EYE, true), 3);
                    else if (x == 2)
                        level.setBlock(pos.offset(x, 0, z), portal_frame.defaultBlockState().setValue(HorizontalDirectionalBlock.FACING, Direction.WEST).setValue(SenterianPortalFrameBlock.HAS_EYE, true), 3);
                } else if (Math.abs(x) < 2) {
                    if (z == -2)
                        level.setBlock(pos.offset(x, 0, z), portal_frame.defaultBlockState().setValue(HorizontalDirectionalBlock.FACING, Direction.SOUTH).setValue(SenterianPortalFrameBlock.HAS_EYE, true), 3);
                    else
                        level.setBlock(pos.offset(x, 0, z), portal_frame.defaultBlockState().setValue(HorizontalDirectionalBlock.FACING, Direction.NORTH).setValue(SenterianPortalFrameBlock.HAS_EYE, true), 3);
                }
            }
