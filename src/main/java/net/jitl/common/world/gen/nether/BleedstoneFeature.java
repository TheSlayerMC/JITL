...
public class BleedstoneFeature extends Feature<NoneFeatureConfiguration> {

    public BleedstoneFeature(Codec<NoneFeatureConfiguration> c) {
        super(c);
    }
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        if (!level.isEmptyBlock(blockpos)) {
            ...
            if (!blockstate.is(Blocks.NETHERRACK) && !blockstate.is(Blocks.BASALT) && !blockstate.is(Blocks.BLACKSTONE)) {
                ...
                level.setBlock(blockpos, JBlocks.BLEEDSTONE_BLOCK.get().defaultBlockState(), 2);
                for(int i = 0; i < 1500; ++i) {
                    ...
                    if (level.getBlockState(blockpos1).isAir()) {
                        ...
                        for(Direction direction : Direction.values()) {
                            if(level.getBlockState(blockpos1.relative(direction)).is(JBlocks.BLEEDSTONE_BLOCK.get()))
                                j++;
                        ...
                        if(j == 1)
                            level.setBlock(blockpos1, JBlocks.BLEEDSTONE_BLOCK.get().defaultBlockState(), 2);
                    }
                }
                return true;
            }
