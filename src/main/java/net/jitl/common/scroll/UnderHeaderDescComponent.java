...
public class UnderHeaderDescComponent implements IDescComponent {
    ...
    public void drawContentPart(GuiGraphics matrixStack, int x0, int y0, int width) {
        ...
        drawImage(x0 + 1 + (int) ((double) width / 2 - ((double) logoWidth * requestedHeight / logoHeight) / 2), y0, 192, 0, logoWidth * requestedHeight / logoHeight, requestedHeight, logoWidth, logoHeight, matrixStack.pose());
        int lastWidth = width - logoWidth * requestedHeight / logoHeight;

        drawImage(x0, y0, 1, 136, (int) ((double) lastWidth / 1.85 - indentFromLogo) + 2, requestedHeight, arrowWidth, arrowHeight, matrixStack.pose());
        drawImage(x0 + width - (int) ((double) lastWidth / 1.85 - indentFromLogo), y0, 127, 136, (int) ((double) lastWidth / 1.85 - indentFromLogo) + 2, requestedHeight, arrowWidth, arrowHeight, matrixStack.pose());
